...
- **cmd2 Return Value Bug**: Fixed critical bug where all shell commands returned `True` instead of `None`, causing the shell to exit after every command. In cmd2, `return True` signals the command loop to exit (like `quit`), while `return None` continues normally. All `do_*` methods across 10 command files now correctly return `None`:
  - `settings.py`, `nodes.py`, `membership.py`, `edges.py`, `filters.py`
  - `paths.py`, `marking.py`, `queries.py`, `connection.py`, `api.py`
  - Special fix in `marking.py`: Changed `return not any_failed` to `return None` (expression evaluated to `True` on success)

- **Test Assertions**: Updated `test_shell_connected.py` assertions from `assert result is True` to `assert result is not True` since commands now return `None`

### Changed

- **Documentation**: Updated CLAUDE.md and CONTRIBUTING.md with cmd2 return value rules explaining that `return True` exits the shell while `return None` continues normally

## [3.0.0] - 2026-01-10

### Changed

- **BREAKING: Interactive Shell Interface**: Complete refactor from CLI tool to interactive shell (cmd2-based):
  - Launch with `python -m runoff [-p password]` to enter shell
  - All operations now performed via shell commands instead of CLI flags
  - Persistent session with connection reuse across commands
  - Tab completion for commands, node names, and settings
  - Command history saved across sessions (~/.config/runoff/history)
  - Script execution support (`@script.txt`)

  **Migration from CLI to Shell:**
  | Old CLI | New Shell |
  |---------|-----------|
  | `python -m runoff -p pass -a` | `run all` |
  | `python -m runoff -p pass --acl --adcs` | `run acl adcs` |
  | `python -m runoff -p pass --path-to-da USER` | `path da USER` |
  | `python -m runoff -p pass --kerberoastable` | `kerberoastable` |
  | `python -m runoff -p pass -o USER -a` | `mark owned USER` then `run all` |
  | `python -m runoff -p pass -d CORP.LOCAL -a` | `set domain CORP.LOCAL` then `run all` |
  | `python -m runoff -p pass --abuse --acl` | `set abuse on` then `run acl` |

- **Command Structure**: Shell commands organized by category:
  - **Connection**: `connect`, `disconnect`, `reconnect`, `status`
  - **Settings**: `set`, `show`, `domains`, `reset`
  - **Node Operations**: `info`, `search`, `investigate`
  - **Path Finding**: `path da|dc|to`
  - **Marking**: `mark owned|tier-zero`, `unmark`, `owned`, `tierzero`, `clear owned|db`
  - **Membership**: `members`, `memberof`, `adminto`, `adminof`, `sessions`
  - **Edges**: `edges from|to`
  - **Queries**: `run <category>`, `query <name>`, `queries`
  - **Quick Filters**: `kerberoastable`, `asrep`, `unconstrained`, `nolaps`, `computers`, `users`, `spns`, `quickwins`, `audit`, `stats`
  - **API**: `auth`, `ingest`, `clear db`

### Added

- **Shell State Management**: `ShellState` class holds session state with domain SID caching for reliable filtering
- **Interactive Help**: `help` shows categorized commands, `help <cmd>` shows detailed usage
- **Aliases**: `t0` (tierzero), `kerb` (kerberoastable), `inv` (investigate), hyphenated variants
- **Shell Prompt**: Dynamic prompt showing connection status, domain filter, and owned count

### Fixed

- **Domain Filtering Consistency**: Implemented flexible OR-based domain filtering that handles BloodHound data inconsistencies where objects have different `domain` properties than Domain node names. Queries now match by:
  - Exact `domain` property match
  - Name suffix match (e.g., `USER@CORP.LOCAL` matches domain `CORP.LOCAL`)
  - Domain SID match (when available)

- **Domain Stats Zero Results**: Fixed `stats` command showing 0 results when domain filter was set but objects had inconsistent domain properties

### Removed

- **CLI Module**: Removed `runoff/cli/` directory (parser.py, main.py, formatter.py, completion.py)
- **CLI Flags**: All `--flag` style arguments replaced with shell commands
- **Argcomplete**: Shell completion now provided by cmd2 instead of argcomplete
...
- **Unix Timestamp Display**: Added automatic formatting of Unix epoch timestamps to human-readable dates (YYYY-MM-DD) in table output. Timestamps like `1648834256.0` now display as `2022-04-01`. Added `format_timestamp()` and `is_unix_timestamp()` utilities in `runoff/core/utils.py`
...
- **`--abuse` Flag**: Re-implemented abuse command display with simplified, Linux-first approach:
  ...
  - Example: `runoff -p 'pass' --acl --abuse`
...
- **Abuse Commands Functionality**: Removed all `--abuse` command functionality and attack templates:
  ...
  - Removed `runoff/abuse/` directory with 58 YAML attack templates
  - Removed abuse command display from `--investigate` output
  - Removed `~/.runoff/abuse.conf` auto-loading
...
- **`--ingest-history` Command**: View file upload/ingest history from BloodHound CE API:
  ...
  - Example: `runoff --ingest-history`, `runoff --ingest-history --json`

- **BloodHound CE API Integration**: New API client for BloodHound CE operations (no Neo4j password required):
  - `--auth` - Authenticate to BloodHound CE and store API token in `~/.config/runoff/runoff.ini`
...
- **BloodHound CE API: Data Ingestion**: New `--ingest` command to upload collector data directly via API:
  ...
  - Example: `runoff --ingest bloodhound_data.zip computers.json`
...
- **BloodHound CE API: Database Clearing**: New `--clear-database` command to delete data from BloodHound CE via the API:
  ...
  - Example: `runoff --clear-database --delete-all --yes`
...
- **`--audit` Consolidated Security Audit**: New command for quick security hygiene assessment:
  ...
  - Example: `runoff -p 'pass' --audit --json`
...
- **Full JSON/CSV/HTML output format support for all CLI commands**: Previously only `-a` (run all queries) supported output formats. Now 21 additional commands support `--json`, `--csv`, and `--html` output:
  ...
  - Example: `runoff --computers --json` returns `[{"name": "DC01.CORP.LOCAL", "os": "Windows Server 2022", ...}]`
...
- **JSON/CSV output for `--stats` command**: The `--stats` flag now properly outputs structured data when combined with `--json` or `--csv`:
  ...
  - Example: `runoff --stats --json` returns `{"domain": "CORP.LOCAL", "users": {...}, "risk": {"score": 40, ...}}`
...
[Unreleased]: https://github.com/Real-Fruit-Snacks/runoff/compare/v3.0.1...HEAD
[3.0.1]: https://github.com/Real-Fruit-Snacks/runoff/compare/v3.0.0...v3.0.1
[3.0.0]: https://github.com/Real-Fruit-Snacks/runoff/compare/v2.5.0...v3.0.0
[2.5.0]: https://github.com/Real-Fruit-Snacks/runoff/compare/v2.3.0...v2.5.0
[2.3.0]: https://github.com/Real-Fruit-Snacks/runoff/compare/v2.2.0...v2.3.0
[2.2.0]: https://github.com/Real-Fruit-Snacks/runoff/compare/v2.1.1...v2.2.0
[2.1.1]: https://github.com/Real-Fruit-Snacks/runoff/compare/v2.1.0...v2.1.1
[2.1.0]: https://github.com/Real-Fruit-Snacks/runoff/compare/v2.0.0...v2.1.0
[2.0.0]: https://github.com/Real-Fruit-Snacks/runoff/compare/v0.3.0...v2.0.0
[0.3.0]: https://github.com/Real-Fruit-Snacks/runoff/compare/v0.2.0...v0.3.0
[0.2.0]: https://github.com/Real-Fruit-Snacks/runoff/compare/v0.1.0...v0.2.0
[0.1.0]: https://github.com/Real-Fruit-Snacks/runoff/releases/tag/v0.1.0
