...
class _DriverPool:
    """Cache Neo4j drivers by connection parameters for reuse.

    The Neo4j Python driver already manages internal connection pooling,
    but creating a new Driver per command adds overhead. This pool allows
    multiple BloodHoundCE instances to share the same underlying driver.
    """

    _STALE_SECONDS = 300  # 5 minutes

    def __init__(self):
        self._drivers: dict[tuple[str, str, str], tuple[Any, float]] = {}  # (uri, username, pw_hash) -> (driver, last_used)
        self._lock = threading.Lock()

    def get_or_create(self, uri: str, username: str, password: str) -> Any:
        """Get an existing driver or create a new one."""
        pw_hash = hashlib.sha256(password.encode()).hexdigest()[:16]
        key = (uri, username, pw_hash)
        with self._lock:
            entry = self._drivers.get(key)
            if entry is not None:
                driver, last_used = entry
                if time.monotonic() - last_used < self._STALE_SECONDS:
                    self._drivers[key] = (driver, time.monotonic())
                    return driver
                try:
    ...
    def _remove_locked(self, key: tuple[str, str, str]) -> None:
        """Remove and close a driver. Must be called with _lock held."""
        entry = self._drivers.pop(key, None)
        if entry is not None:
            driver, _ = entry
            try:
                driver.close()
            except Exception:
                logging.getLogger(__name__).debug("Error closing Neo4j driver", exc_info=True)
...
class BloodHoundCE:
    ...
    def __init__(self, uri: str, username: str, password: str, debug: bool = False):
        ...
        self._structured_emitted = False

    def connect(self) -> bool:
        """Establish connection to Neo4j"""
        try:
            self.driver = _driver_pool.get_or_create(self.uri, self.username, self.password)
            self._owns_driver = False  # Pool owns the driver
            self.password = None
            return True
        except AuthError as e:
            console.print(f"[error][!] Authentication failed: {e}[/error]")
        except ServiceUnavailable as e:
            console.print(f"[error][!] Neo4j service unavailable: {e}[/error]")
        except Neo4jError as e:
            console.print(f"[error][!] Neo4j error: {e}[/error]")
    ...
    def close(self):
        ...
        If the driver was obtained from the pool, it is not closed here
        (the pool manages its lifecycle). Only directly-created drivers
        are closed.
        """
        if self.driver:
    ...
    def run_query(self, query: str, params: Optional[dict] = None) -> list:
        ...
        try:
            ...
            console.print(f"[error][!] Unexpected error during query: {e}[/error]")
            raise

        elapsed = time.time() - start_time
        if self.debug:
            console.print(f"[text.dim][DEBUG] Query completed in {elapsed:.2f}s[/text.dim]")
    ...
    def get_group_members(self, group_name: str) -> list:
        ...
        if _has_wildcard(group_name):
            ...
            RETURN DISTINCT g.name AS group, m.name AS member, {node_type('m')} AS type,
                   ...
                   m.domain AS domain, m.objectid AS objectid
    ...
    def get_member_of(self, principal: str) -> list:
        ...
        if _has_wildcard(principal):
            ...
            RETURN DISTINCT n.name AS principal, g.name AS group_name,
                   ...
                   g.domain AS domain, g.objectid AS objectid,
    ...
    def get_audit_results(self, domain: Optional[str] = None) -> Dict[str, Any]:
        ...
        u_where = "WHERE toUpper(u.domain) = toUpper($domain)" if domain else ""
        u_and = "AND toUpper(u.domain) = toUpper($domain)" if domain else ""
        ...
        AND NOT u.name STARTS WITH 'KRBTGT' {u_and}
        ...
        query = f"""
        MATCH (u:User {{dontreqpreauth: true, enabled: true}}) {u_where}
        ...
        WHERE c.operatingsystem =~ '.*(2000|2003|2008|XP|Vista|Windows 7|Windows 8).*' {c_and}
        RETURN c.name AS name, c.operatingsystem AS os
        ...
        query = f"""
        MATCH (c:Computer {{enabled: true}})
        WHERE (c.haslaps IS NULL OR c.haslaps = false) {c_and}
        ...
        WHERE u.objectid ENDS WITH '-501' {u_and}
        ...
        query = f"""
        MATCH (u:User {{enabled: true, pwdneverexpires: true, admincount: true}}) {u_where}
        ...
        WHERE (g.objectid ENDS WITH '-512' OR g.objectid ENDS WITH '-519') {u_and}
...
def close_pool() -> None:
    """Close all pooled Neo4j drivers. Call during application shutdown."""
