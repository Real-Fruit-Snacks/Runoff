...
class BloodHoundAPI:
    ...
    def _request(
        ...
        Args:
            ...
            retries: Number of retries for transient 5xx errors

        Returns:
            Response object

        Raises:
            BloodHoundAPIError: If the request fails after all retries
        """
        import time
        ...
        for attempt in range(retries + 1):
            # Build authentication headers (fresh per attempt for timestamp)
            headers = build_auth_headers(method, endpoint, self.token_id, self.token_key, body)
            headers["Content-Type"] = content_type
            headers["User-Agent"] = "runoff/1.0"
        ...
        raise BloodHoundAPIError(f"Request failed after {retries} retries: {last_error}")
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if config.debug_mode:
                    ...
                    console.print(f"[text.dim]Debug: Job status response: {status}[/text.dim]")
                if callback:
                    callback(status)
