"""HTML report generation for runoff"""
...
<html>
<head>
    <title>Runoff Report - {date}</title>
    <meta charset="utf-8">
    <style>
        * {{ box-sizing: border-box; }}
        ...
        .high {{ background: #eba0ac; color: #1e1e2e; }}
        .medium {{ background: #fab387; color: #1e1e2e; }}
        .low {{ background: #f9e2af; color: #1e1e2e; }}
        .info {{ background: #7f849c; color: #1e1e2e; }}
        .finding-content {{
            overflow: hidden;
        ...
        .stat.active {{ box-shadow: 0 0 0 3px #89b4fa; }}
        .stat-value {{
            font-size: 2em;
            font-weight: bold;
        }}
        ...
        .pagination button:hover {{ background: #313244; }}
...
<body>
    ...
    <script>
        ...
        function exportCSV() {{
            ...
            a.href = url;
            a.download = 'runoff-findings.csv';
            a.click();
        ...
        if count > 0:
            ...
            if result_data and len(result_data) > 0:
                ...
                if len(result_data) > 100:
                    truncation_note = (
                        f'<p style="color: #f9e2af;">Showing 100 of {len(result_data)} results</p>'
                    )
...
<body>
    ...
    <footer>Generated by Runoff v3.1.0</footer>
...
def generate_simple_html(
    ...
    filename = _re.sub(r"[^a-zA-Z0-9_-]", "", title.lower().replace(" ", "_"))[:30]
