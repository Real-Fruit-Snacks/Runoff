...
def get_domain_stats(bh: BloodHoundCE, domain: str | None = None, severity: Severity = None) -> int:
    ...
    # ADCS stats - use same flexible filter
    if domain:
        adcs_filter = """WHERE (
            toUpper(n.domain) = toUpper($domain)
            OR toUpper(n.name) ENDS WITH toUpper($domain_suffix)
    ...
    # Use flexible filter for computer matching
    if domain:
        dc_domain_filter = """AND (
    ...
    WHERE g.objectid ENDS WITH '-516' {dc_domain_filter}
    ...
    if config.output_format == "table":
        ...
        rich_style = style_map.get(rating, "text.dim")
        console.print(f"    Risk Score: [{rich_style}]{score}/100 ({rating})[/{rich_style}]")
        console.print()
